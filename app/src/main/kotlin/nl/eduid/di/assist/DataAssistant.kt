...
class DataAssistant @Inject constructor(
    ...
    private suspend fun forwardWithFallback(
        result: Result<UserDetails>,
        knownDetails: UserDetails,
        operation: String = "",
    ) = result.fold(
        onSuccess = {
    ...
    private suspend fun currentCachedSettings(): UserDetails? =
        when (val current = cachedDetails.firstOrNull()) {
